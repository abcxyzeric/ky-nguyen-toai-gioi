...
- Đã đưa trạng thái bật/tắt của toàn bộ 24 Regex về khớp tuyệt đối với bản gốc; đã khôi phục rule `Xúc xắc (raw GitHub)` về bật như bản gốc. PNG `11_vi.png` đã được đóng gói lại từ JSON mới, kiểm tra CRC/JSON nhúng thành công và đã push cùng các tệp `dist`.
- Sau khi push, 9/9 URL giao diện đang bật đều phản hồi HTTP 200 và có script hợp lệ. Sáu URL jsDelivr dự phòng (gồm cả xúc xắc đang bật) bị cache bản `@main` cũ, nên đã ghim riêng vào commit `b4f75b0` đã kiểm tra; các URL ghim này đều HTTP 200 và phân tích cú pháp hợp lệ. Link hệ thống đặc tính vẫn tắt đúng như bản gốc và cùng đường dẫn gốc 404 vì tính năng chưa hoàn thiện.
- Đã xác nhận lại PNG sau push trực tiếp từ GitHub: HTTP 200, một metadata `chara`, tag khởi đầu khớp Regex và tất cả URL jsDelivr trong card trỏ đúng commit đã kiểm tra.
